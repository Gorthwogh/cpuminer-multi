...
#include "cryptonote/cryptonight_gr.h"
...
static void getAlgoString(void *mem, unsigned int size, uint8_t* selectedAlgoOutput, int algoCount) {
  ...
  unsigned char j = 0;
  bool selectedAlgo[HASH_FUNC_COUNT];
...
void gr_hash(void* output, const void* input) {
	...
	getAlgoString((char*)&input+4, 64, selectedAlgoOutput, 15);
	getAlgoString((char*)&input+4, 64, selectedCNAlgoOutput, 6);
	int i;
	for (i = 0; i < 18; i++)
	{
		uint8_t algo;
...
//thr_id, &work, max_nonce, &hashes_done);
