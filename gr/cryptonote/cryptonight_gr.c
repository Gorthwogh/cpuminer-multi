...
static void do_blake_hash_gr(const void* input, size_t len, char* output) {
    blake256_hash((uint8_t*)output, input, len);
}

void do_groestl_hash_gr(const void* input, size_t len, char* output) {
    groestl(input, len * 8, (uint8_t*)output);
}

static void do_jh_hash_gr(const void* input, size_t len, char* output) {
    int r = jh_hash(HASH_SIZE * 8, input, 8 * len, (uint8_t*)output);
    assert(SUCCESS == r);
}

static void do_skein_hash_gr(const void* input, size_t len, char* output) {
    int r = skein_hash(8 * HASH_SIZE, input, 8 * len, (uint8_t*)output);
    assert(SKEIN_SUCCESS == r);
}

static void (* const extra_hashes[4])(const void *, size_t, char *) = {
    do_blake_hash_gr, do_groestl_hash_gr, do_jh_hash_gr, do_skein_hash_gr
};
...
void cryptonight_hash_gr(const char* input, char* output, uint32_t len, int variant) {
#if defined(_MSC_VER)
...
void cryptonight_fast_hash_gr(const char* input, char* output, uint32_t len) {
